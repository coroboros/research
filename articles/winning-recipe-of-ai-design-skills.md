...
**4. Concrete values on commodity axes, freedom on identity axes.** elaya's closed tables are the extreme case (`SKILL.md:172,201`). The generalizable version derives identity from the subject's world instead of from a template, which is what impeccable's "already spent" rule forces (`new-work.md:70`): after rejecting the reflex, the second choice must come from somewhere, and the only somewhere left is the brief.

**5. One signature moment, exactly one.** elaya's mandatory B11 reveal (`skills/landing-page-design/SKILL.md:341`). Anthropic's compressed version quotes Chanel: "before leaving the house, take a look in the mirror and remove one accessory" (`plugins/frontend-design/skills/frontend-design/SKILL.md:43`).

**6. Content realism.** elaya: "Never ship filler" (`:301`). OpenAI's rubric bans faked assets outright: "Never fake visible assets with ASCII, prose, text symbols, emoji, placeholder boxes, CSS art, div art …" ([openai/plugins](https://github.com/openai/plugins) `plugins/product-design/references/critical-overrides.md:58`).
...
**Rung 2: gate artifacts.** OpenAI's `design-qa` writes a `design-qa.md` whose "final result must be exactly passed or blocked" (`plugins/product-design/skills/design-qa/SKILL.md:168`) and instructs "Do not let the build skill hand off as done" when comparison is impossible (`:19`). huashu ships a shell hook that refuses long renders without its `direction-approved.md`, active only once the user wires it into `settings.json`, with `SKIP_DESIGN_GATE=1` as an auditable bypass (`scripts/design-gate-hook.sh:12,14`). The artifact makes skipping the gate a visible act instead of a silent one.
...
- **elayadesign/ai-design-skills** · 403 lines, 1 file — the case study above, whole. The highest output-per-line ratio in the corpus, at the price of a house style every output shares.
...
- **NousResearch hermes-agent, claude-design** · 650 lines plus two siblings — the compositional theory above, split into process (`claude-design`), brand mimicry (`popular-web-designs`, 54 templates), and token specs (`design-md`); no other system separates its concerns this cleanly.
- **pbakaus/impeccable** · 5,305 prose lines plus a large executable layer — contract, "already spent", 18 craft-floor rules the brief must earn back (one an outright ban), 61 detector rules, comp-diffing, isolated reviewers. The reference implementation for measured verification.
...
- **alchaincyf/huashu-design** · 13,294 Markdown lines (Chinese) — three direction drafts from three parallel, mutually isolated subagents ("independent contexts … to avoid convergence", translated from `SKILL.md:290`), a user pick recorded in `direction-approved.md`, and the opt-in gate hook above.
...
- **nextlevelbuilder/ui-ux-pro-max-skill** · 6,943 prose-and-script lines plus ~4,300 of CSV — a searchable recommendation database (192 product reasoning profiles, 74 font pairings, queried by script). The corpus's control group: it recommends the exact center of the distribution the other systems exist to escape.
