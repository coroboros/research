...
sources:
  ...
  - "https://thefwa.com/cases/cartier-watches-wonders-2025"
...
| Mechanic | Named winner | Award record | Stack | What it ships |
|---|---|---|---|---|
| WebGL displacement / flowmap image transition | Igloo Inc | Awwwards SOTD 23 Jul 2024, 7.92, Developer Award 7.66; Site of the Year 2024 and Developer Site of the Year 2024 | Three.js + custom WebGL shaders + Svelte + GSAP + Vite + `three-mesh-bvh` | "tech displacement" — camera and scene-transition-driven, applied to "a few elements and materials", not proportional to Lenis scroll velocity, and a full 3D scene transition rather than a 2D image flowmap |
...
| Pinned scroll-scrubbed video / image sequence | Cartier Watches & Wonders (loose match; two editions) | 2025 edition: Awwwards SOTD 18 Aug 2025, 7.64, Developer Award 7.55, Site of the Month Aug 2025 ([awwwards](https://www.awwwards.com/sites/cartier-watches-wonders-2025)); CSSDA Website of the Day 16 Jul 2025, 8.39 ([cssda](https://www.cssdesignawards.com/sites/cartier-watches-and-wonders-2025/47763)); FWA of the Day 7 Jul 2025 ([FWA](https://thefwa.com/cases/cartier-watches-wonders-2025)). 2026 edition: Awwwards SOTD 25 May 2026, 7.53, Developer Award 7.85 ([awwwards](https://www.awwwards.com/sites/cartier-watches-wonders-2026)) | Three.js + GLSL + GSAP + Lenis | Lenis-driven scroll transitions across six (stated, [webgpu.com](https://www.webgpu.com/showcase/cartier-watches-and-wonders-immersive-garden/)) 3D rooms, the 2025 edition as read in [corporate-luxury](../archetypes/corporate-luxury.md) — a WebGL camera scrub, not an Apple-style `currentTime` scrub; the 2026 edition is carried for its award line only |
...
The Lando Norris site is a genuine Awwwards winner: SOTD 17 Nov 2025 at 8.18, by OFF+BRAND, on a Webflow/GSAP/WebGL/3D/Rive stack. Site of the Year 2025 and Site of the Year Users' Choice 2025 are confirmed on the [Awwwards annual winners page](https://www.awwwards.com/annual-awards/winners); the CSSDA and FWA legs come from the OFF+BRAND case study and were not checked against those bodies' listings ([`../winners/site-of-the-year-contenders.md`](../winners/site-of-the-year-contenders.md)).

The cursor-trailing mask attributed to it does not survive. A word-level scan of the OFF+BRAND case study finds "mask", "reveal", "cursor", "pointer", "hover", and "shader" all absent; motion is described only as "speed-inspired animations" and "cinematic scrolling." The Awwwards "Hover Trails and Mask Reveal on Cursor Move" page by Marga Navarro is tagged ELEMENT, carries no award badge or score, and sits on the `/inspiration/` path, structurally distinct from the juried `/websites/sites_of_the_day/`. No award-body or studio source attributes a "Mask Reveal" to the site. The live read finds `getComputedStyle(body).cursor === "auto"` and no follower element ([`../archetypes/immersive-cinematic.md`](../archetypes/immersive-cinematic.md)); the mechanic remains demo-only.
...
- **MDN's `@supports` fallback proves authors must guard for unsupported browsers.** Unsupported: the cited MDN pages carry no such requirement.
- **CSS scroll-triggered animations are Chromium-gated at launch and require a production fallback.** Unsupported: the capability is confirmed (Chrome 145), the cross-browser status is unsettled rather than established as gated.
...
FWA legs are the weakest link throughout: for ERA, Ever, and Montfort the FWA badge is JS-rendered and could not be read from the raw HTML. Montfort's FWA leg has no located award page at all. The Cartier Watches & Wonders 2026 edition was read for its award line only; the six-room scroll mechanics in the table belong to the 2025 edition.
...
Whether the Awwwards hover-trails page carries an award citation is unresolved; the page is inspiration-tier either way and cannot back the cursor-trailing mask.

- Unverified: whether any named winner ships the strict scroll-velocity-coupled version of displacement or RGB-shift (Lenis velocity mapped to skew or shader uniform), as opposed to the scene-transition usage on Igloo Inc.
- Unverified: whether a named winner's single signature is a clip-path/SVG text-mask reveal, a GSAP SplitText climax, a sticky horizontal scroll section, or an inset clip-path wipe; each needs a dedicated hunt before a demo-only verdict.
- Unverified: the FWA-of-the-Day legs for ERA, Ever, and Montfort from a directly-read award page rather than a `/cases/` page plus studio claim.
- Unverified: whether the emerging scroll-triggered model (Chrome 145) is shippable with a known fallback; its capability is confirmed, its cross-browser status is not.
- Unverified: the 2026 production fallback pattern for `animation-timeline` given it is not Baseline, pure progressive enhancement with a static Firefox base or GSAP ScrollTrigger parity for the reveal.
- Unverified: where the jury-tier line falls between content-reveal motion (fire-once, persistence-governed) and decorative ambient motion (reversible scrubbing tolerated).
