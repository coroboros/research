...
| `oklch()` | Widely available since May 2023; Widely recorded 9 Nov 2025 (~93–95% reach) | adopt as default, no `@supports` guard |
| `content-visibility` | Newly available 15 Sep 2025 (Safari 26 was the last engine) | shippable by default — unsupported engines ignore it and render normally |
| Same-document View Transitions + `view-transition-class` | Newly available 14 Oct 2025 (Firefox 144 completed interop) | progressive enhancement, feature-detect — the least settled cell here |
| Relative color syntax `oklch(from …)` | Newly available Sep 2024 | progressive enhancement, not Widely |
...
The recognizable code tells are an **inference** from the confirmed baselines above. No citable practitioner source naming them was found, so the list carries no evidentiary weight of its own:

- inline literals instead of tokens
- `px` instead of `rem`
- hex or `rgb()` instead of `oklch()`
- div-soup instead of semantic HTML
- missing `prefers-reduced-motion`
- missing `@supports` guards
- un-factored repetition
- no container queries
- one generic fade-in everywhere

## Could not verify

Verdicts are pinned to mid-2026. Newly-available features (`content-visibility` 15 Sep 2025, same-document View Transitions 14 Oct 2025, relative color Sep 2024) have only 9 to 12 months of cross-engine interop.

Four areas were **not verified here** and are not established as baseline:
...
4. **2025's Chrome-first feature set.** Invoker commands (Chrome 135), dialog light dismiss (Chrome 134), scroll-state queries (Chrome 133), `moveBefore` (Chrome 133), and expanded range syntax (Chrome 142) carry version numbers; whether they are cross-browser Baseline is unconfirmed either way.
