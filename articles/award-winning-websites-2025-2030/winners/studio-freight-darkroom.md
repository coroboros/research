...
- **Runtime behavior** (mounted WebGL canvases, scroll scrubbing, hover states): no browser execution was used; claims like the 300vh scroll-pinned sample and the char-split hero are inferred from SSR markup and CSS (`h-[300vh] sticky`, `.char{contain:layout style}`), not from lived interaction. No `<canvas>` exists in any SSR payload; WebGL is client-mounted.
