...
**Nav bar surface** `[JS + CSS]`. `.header` is `position: fixed; z-index: 889` with a separate `.header_bg` layer whose `background-color: base-0--100`. That background is **held at `opacity: 0` until scroll passes 1000px**, then fades to 1 (`durS`, `Out`). Its surface color is the current section's own background, `#a89474` (tan) over light sections and `#2c2824` (dark brown) over dark ones, because the theme system reassigns the token on scroll. **No border-bottom of any color. No `backdrop-filter` blur.** The header also hides on scroll-down (`yPercent: -100`, `durM`) and reappears on scroll-up (`yPercent: 0`), ignoring sub-40px jitter and force-showing within 160px of the footer.
