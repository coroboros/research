---
title: "Route Transitions on Winners"
date: "2026-07-30"
author: "Coroboros"
...
**Corpus.** Fifteen sites read live — eleven award-verified, four studio exemplars (Cuberto, Locomotive, Immersive Garden, Active Theory) carried for their route machinery. HTML plus linked CSS by `curl`, two JS bundles decompiled by grep, Active Theory driven in a real browser.

| Site | Award | Router / stack | Read |
|---|---|---|---|
| **Cuberto** — cuberto.com | studio exemplar, not award-verified | custom SPA (`bundle.js?v=5.5.0b8`) | DOM + CSS |
...
| **Truekind Skincare** — truekindskincare.com | Site of the Day (SOTD) Apr 2025 | Nuxt + GSAP Flip | DOM + CSS |
| **Siena Film Foundation** — siena.film | Site of the Month (SOTM) Mar 2025 | Taxi.js + GSAP on a Webflow base | DOM + JS |
| **Terminal Industries** — terminal-industries.com | SOTM Sept 2025 | Nuxt native app/page transition | DOM + CSS |
| **Lando Norris** — landonorris.com | SOTD 17 Nov 2025 (8.18); Site of the Year (SOTY) 2025 per the OFF+BRAND case study, unverified | Taxi.js + Rive on a Webflow base | DOM |
...
| **Stefan Vitasović** — stefanvitasovic.dev | SOTD Sept 2025, Dev Award | Next.js | DOM |
| **Gabriel Contassot** — gabrielcontassot.com | SOTD Apr 2024 | Taxi.js | DOM |
| **Mat Voyce** — matvoyce.tv | SOTD + FWA | Next.js, custom wipe | DOM + CSS |
| **Ponpon Mania** — ponpon-mania.com | SOTM Oct 2025 | Nuxt, narrative loader | DOM |
| **Eloy Benoffi** — eloyb.design | GSAP SOTD + CSSDA | custom GSAP | DOM |
...
Router split: **Taxi.js ×3** (Siena, Lando, Contassot) · **Barba.js ×2** (Locomotive, Dennis) · **Nuxt-native ×4** (Immersive Garden, Truekind, Terminal, Ponpon) · **Next.js ×3** (Robin, Stefan, Mat Voyce) · **custom SPA ×3** (Cuberto, Eloy, Active Theory). **Zero of these 15 ship the native View Transitions API as their route transition** — every route transition in the census is JS-orchestrated. Outside the census, Cyd Stumpel ships a verified native View-Transition shared-element route ([`../archetypes/spatial-organic.md`](../archetypes/spatial-organic.md)).
...
| 1 cover-in | `.cb-loader-fill` (`background:#fff`) fills the viewport; `.cb-loader-backdrop` `rgba(0,0,0,.3)` dims underneath (JS-driven, observed) | overlay becomes `.active`, `opacity` → 1 over the swap (GSAP, observed); the deeper CSS read gives the *visible* cover as `.app-loader`'s dark `.overlay` variant instead — see below | `opacity` → 1, white sheet in (JS, observed) |
...
Cover panels are the page ground, never a neutral grey: Cuberto `#fff`, Truekind `#fff`. Terminal is the exception — the deeper CSS read of the same build gives the route-change form as `.app-loader`'s dark variant, `.overlay[data-v-068da249]{background:rgba(0,0,0,0.7)}` = `#000000b3`, a dark cover over the cream site rather than a light one ([minimalist](../archetypes/minimalist.md)). Z-index is a decisive `998–999` so nothing bleeds through. Cuberto's stylesheet carries two `.cb-loader` height rules (`100vh` then `100lvh`); the effective value is `100lvh`.
...
Who ships it — Taxi: Siena, Lando Norris, Gabriel Contassot. Barba: Locomotive, Dennis Snellenberg.
...
- **Terminal Industries** — `.app-loader`, its dark `.overlay` variant, and `.app-transition` are all fixed, full-viewport, `z-index: 999` overlays in the same Vue component family, and the deeper CSS read closes the distance further: the route change reuses `.app-loader` itself, one curtain at two amplitudes ([minimalist](../archetypes/minimalist.md)). A winner-verified negative on the same site: "highway" appears only in body copy ("seamlessly connecting highway to warehouse"), never as the Highway.js router. (See the correction below on the exact scope.)
...
Sources re-opened: HTML, CSS, and JS by `curl` as verbatim bytes; Active Theory driven live; docs from `taxi.js.org`, MDN, web.dev, caniuse, and the Barba issue tracker.
...
- **Terminal's route cover is a light overlay over its cream site** — false: the same build's deeper CSS read gives `.overlay[data-v-068da249]{background:rgba(0,0,0,0.7)}` = `#000000b3` as the route-change form, a dark cover, and attributes it to a reuse of `.app-loader` rather than to `.app-transition` alone ([minimalist](../archetypes/minimalist.md)). The `.app-transition` rules quoted above stay as read — the element exists and carries an `.active` state — but the cover's ground was wrong and the element painting it is contested.
- **Siena Film Foundation — SOTM Apr 2025** — false: the authoritative `sites_of_the_month/` listing gives **March 2025** ([editorial](../archetypes/editorial.md)).
- **`inset: 0` compression.** Exact where all four sides are 0 (Truekind's base overlay and preloader). For `.clone-transition` and `.app-transition` the live rule is `left:0; top:0; width:100%; height:100vh|100%` with no explicit right or bottom — functionally equivalent, worth knowing if copying verbatim.
- **De-minified renderings.** Siena's `onLeave` and Mat Voyce's background-loop keyframe are readable renderings of minified and CSS-module-hashed source. Structures and literal values (`B.pages.transitionOut`, `10s linear infinite`, `wght 1000`) match byte for byte; only auto-generated identifiers differ.
