...
- **Winners** — Cuberto (SOTD). The Codrops "Image Trail Effects" set is the canonical implementation, refreshed in 2026 as a gravity variant.
...
- **Winner** — **Exat** typeface, Awwwards SOTD **7.73**, 3 April 2025 — kinetic type whose glyphs respond to pointer distance.
...
- **Winner** — Cuberto, whose `mouse-follower` library powers it. Lando Norris does not belong here: `body { cursor: auto }` with no follower element at all ([live-read](../archetypes/immersive-cinematic.md), [bold-maximal](../archetypes/bold-maximal.md)).
