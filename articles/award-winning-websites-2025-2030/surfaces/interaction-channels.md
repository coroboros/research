...
|---|---|---|---|---|---|---|---|
| FlowFest (winner-verified) | hard-shadow press button, underline-draw link, ±3° card tilt with sticker peel | SVG-drawn H1 whose spans stagger in, chat-cloud typed text (TextPlugin) | default | 4 rainbow arches draw over ≈3 s at load, JS-verified in the [Slater bundle](https://slater.app/14984/39049.js): `animateRainbow`, count 4, duration 2 s, delay 0.5–1 s; the vertical rainbows are 9-path scroll-scrubbed draws; chat mascot idle | photo shuffle carousel | chat-cloud → hero reveal | 5 |
| Eloy (winner-verified / technique) | clone CTA | char-diff swap (TextPlugin `type:"diff"`), scramble nav, RGB-split type | bitmap swap `url(cursor.svg)` | forced-open scramble nav | — | clone field on `.about-section`, 2 clones per 200 px of travel, uncapped ([brutalist](../archetypes/brutalist.md)) | 5 |
...
|---|---|---|---|---|---|
| Cuberto (winner-verified) | dome-fill CTA, 7° skew text-roll links, scaling underline | SplitText headline | contextual-state follower (`mouse-follower`, `::before` rescales per target, `stickDelta: 0.15`, the library default) | — | 4 |
...
- **Igloo's footer particle journey is a confirmed mechanic.** Unsupported: the Awwwards case study describes no footer particle journey ([`../archetypes/experimental.md`](../archetypes/experimental.md#refuted)).
- **FlowFest's C2 runs on SplitText spans.** False: SplitText ships in a script tag and is never registered; the [Slater bundle](https://slater.app/14984/39049.js) registers ScrollTrigger, CustomEase, DrawSVGPlugin, TextPlugin, and Draggable only ([brutalist](../archetypes/brutalist.md)). The C2 channel is real and stays: the H1 is inline `<svg><path>` whose spans stagger in on the loader handoff, and the chat cloud types through TextPlugin. The count of 5 is unaffected.
- **Eloy's clone machine lives in the footer, capped at ≤200 clones on `back.in(1.7)`.** False: the clone field runs on `.about-section`, spawning 2 clones per 200 px of travel, uncapped ([brutalist](../archetypes/brutalist.md), [bold-maximal](../archetypes/bold-maximal.md)).

## Could not verify

- Exat's proximity glyph grid as a winner mechanic, and the Naked City and Joffrey rows, are technique-tier; Lusion and Bruno Simon are verified on copy only. None of these is read from shipped code.
