---
title: "Experimental — Effect Palette, Page Recipe, Aliveness"
date: "2026-07-30"
author: "Coroboros"
...
sources:
  ...
  - "https://www.cssdesignawards.com/woty2023/sites/lusion-v3"
  - "https://www.awwwards.com/annual-awards-2023/"
...
# Experimental — Effect Palette, Page Recipe, Aliveness

Across this corpus **the interaction layer renders inside the WebGL/canvas engine rather than being stamped on DOM elements with CSS**, and the line's overall jury scores cap at 8.0–8.25 — Lusion's 8.25 is the corpus's highest whole-site score. Effect palette, page recipe, and mid-page aliveness for the experimental / art-directed archetype. Parent reference: [Award-Winning Websites — 2025–2030 Reference](../award-winning-websites-2025-2030.md).
...
Evidence tags carry through every section. `(winner-verified)` = live CSS/DOM/JS read against the shipping site. `(awwwards score)` = read from the Awwwards site page. `(shipped)` = observed live or in award-page media, implementation not read. `(technique)` = documented method, typically Codrops. `(single-source)` = one site only; needs a second site before it reads as a rule.

## Corpus

| Site | Award | Jury scores | Stack | URL |
|---|---|---|---|---|
| **Bruno Simon — 2019 build** | Awwwards Site of the Day (SOTD) 11 Nov 2019 + FWA + CSSDA (both belong to this build, not the rebuild) | Design 7.94 / Usability 7.55 / Creativity 8.95 / Content 8.13 / **Overall 8.04** | Three.js + Cannon.js, matcaps, spatial audio; solo dev | `bruno-simon.com` |
| **Bruno Simon — 2025 rebuild** | Awwwards SOTD 21 Jan 2026 (Site of the Month (SOTM) Jan 2026) + Developer Award + Portfolio Honors — no FWA or CSSDA on this entry | Design 8.05 / Usability 7.83 / Creativity 8.62 / Content 8.18 / **Overall 8.11** | same lineage, physics drop-in | `bruno-simon.com` |
| **Igloo Inc** (Abeto) | Awwwards SOTD 23 Jul 2024 + **Site of the Year (SOTY) 2024** + Developer SOTY | Design 8.05 / Usability 7.5 / Creativity 8.31 / Content 7.91 / **Overall 7.92** | Three.js + Svelte + GSAP + Houdini + Blender; entire UI in WebGL | `igloo.inc` |
| **Lusion v3** | Awwwards SOTD 2 Oct 2023 + **Site of the Year 2023**; CSSDA WOTY 2023 nominee | Design 8.26 / Usability 7.95 / Creativity **8.65** / Content 8.26 / **Overall 8.25**; Dev Award 8.41 | Three.js + GSAP; studio flagship | `lusion.co` |
...
| **Obys Agency** | Awwwards **Studio of the Year 2023**, multiple SOTD | — | GSAP; magnetic cursor + kinetic type signature | `obys.agency` |
...
Named from the award record without a live read: **Terminal Industries** SOTM Sep 2025 · **Messenger** SOTD 10 Nov 2025 + Developer Award 8.21 · **Lando Norris / OFF+BRAND** SOTD 17 Nov 2025 (8.18); SOTY 2025 per the OFF+BRAND case study, unverified · **Cartier W&W25 / Immersive Garden** Agency of the Year 2025.

Two evidence tiers stay separate throughout. Awarded sites anchor every recipe. Codrops technical breakdowns supply the exact numbers the awarded sites never publish; a recipe resting only on Codrops is flagged `(technique)`.
...
### Nav bar — no scrolled surface, because there is usually no bar
...
- **Scramble-to-resolve decode — signature.** Headline and label characters cycle through a punctuation-heavy set then settle, running *in parallel* with a clip-path wipe rather than after it. *Verified (Guignand):* charset `A!B@C#D$E%F&G*H?J[K]L{M}N=O+P-QRSTUVWXYZ`; clip-path `inset(0 0% 0 0)`, 0.6s `power2.out`; parent height locked via `getBoundingClientRect().height` before the split to stop reflow. Igloo does the same via SDF-offset scramble. ≥2.
- **Kinetic type on scroll — signature for the typography stack.** Letters scale, split, and morph as the page scrolls, type carrying the layout instead of images. Obys Agency kinetic typography system. `(single-source as a named winner)` — stack-specific, not universal.
...
- **Variable-font weight tied to input** — weight axis driven by mouse or audio amplitude. Igloo syncs particle and audio to motion, adjacent but not the same. `(shipped, implementation unverified)` — ship no exact numbers.
...
- **Magnetic elastic follower — signature.** A dot, ring, or blob trails with spring easing, then snaps onto and scales around hoverable elements, pulling them slightly toward it. The default for typography/agency/portfolio builds. Obys, Cuberto (`mouse-follower`, GSAP v3), Lusion v3. ≥3.
...
- **Generic `fade-up 20px` as the only reveal.** Replaced by scramble plus clip-path, physics, or clip-morph.
- **Spinner or bare `%` counter preloader untied to the concept.**
- **`mix-blend-mode` lag-dot cursor that adds no mechanic.**
...
**Ordered skeleton (Igloo).** Intensity `[n]` is a comparative 1–10 estimate of attention load (defined in `../winners/composition-chains.md`). `real-time in-engine intro (outdoor scene) [8] → per-project zones, each encased in a generated ice block [6–7] → interactive links = particle simulation forming a model per link [10, climax] → closing beat [4, rest]`. The case study describes "each project" generically.
...
**Fits.** Freelance developer and designer portfolios, studio folios, agency work indexes. Aristide `(winner-verified)`, Obys grid variant `(winner-verified)`.
...
| Ground objects | 2 | rise from ground (physics drop-in) | staggered | physics (Rapier) |
...
- **Obys** `(winner-verified)`. Contact-first: a one-sentence studio blurb, `info@obys.agency`, the persistent "CET 00:00 AM" clock, "All rights reserved. ©2026 Obys".
...
- **No winner opens on a card grid** with a conventional hero band above feature cards. The first fold is a world, an in-engine statement, or a giant-type index.
- **No stock photography.**
- **No concept-less spinner or bare-`%` preloader.**
- **No lag-dot cursor as a default reflex.** Aristide's body cursor computes to `auto` in the rendered DOM, with the bottom-centered `.e` indicator as the only follower; Bruno omits a cursor because the car is the pointer.
- **No feature-list or adjective-stack copy.**
- **No conventional footer nav sitemap.** The footer is a payoff, a signed identity block, or contact-first chrome.
...
### Smooth scroll — bespoke or native, never Lenis at this tier

Lenis and Locomotive, the corporate/editorial default smoothers, are absent from every readable bundle on this line.
...
### Tier anti-signals — what the >8 scorers do not do mid-page

- **No conventional scrolling prose middle** in the spectacle or index registers. It is virtualized into the engine or replaced by a hover-live index. A 7.2–7.4 build's dead prose section is itself slightly off-archetype here.
- **No hover on running body prose.**
- **No Lenis or Locomotive** as the smoothing layer.
- **No GSAP ScrollTrigger scrub as the mid-page spine** in the readable index build.
- **No generic `fade-up 20px`.** Reveals are masked `translate3d` plus SplitText letter/word/line staggers, or nothing.
- **Quiet register: no motion decor on the reading column.** Aliveness is typographic — marginalia, sidenotes, hanging figures, randomized stamps.
...
- **"`d.css` is ~7.2KB minified"** — false: the live file is **9,485 bytes** (~9.5KB). Neither the minified size nor a gzip estimate lands on 7.2KB.
- **"Aristide ships WebP assets"** — unsupported: `window._A.webp:false` in the fetched context. WebP is served on capability detection, with no positive proof read.
...
- **"Bruno's 8.04 overall belongs to the 2025 rebuild"** — false: Design 7.94 / Usability 7.55 / Creativity 8.95 / Content 8.13 / Overall 8.04 belong to the **2019 build**, SOTD 11 Nov 2019. The 2025 rebuild is a separate entry — SOTD 21 Jan 2026, Design 8.05 / Usability 7.83 / Creativity 8.62 / Content 8.18 / Overall 8.11. Both sit below 8.5, so the ceiling finding is unaffected.
- **"Aristide SOTM Jun 2021, overall ≈8.02"** — half false, and the SOTM half is restored: the overall is **8.01**, not ≈8.02, and the entry is SOTD **24 Jun 2021** — but the SOTM is real. Awwwards carries a dedicated announcement page for it (`…aristide-benoist-portfolio-2021-wins-site-of-the-month-june-2021.html`) and the profile records 5 SOTM ([`../winners/studio-unseen-benoist.md`](../winners/studio-unseen-benoist.md)). The earlier refutation rested on the badge's absence from the site page, which an announcement page outranks. Portfolio 2021 holds SOTD 24 Jun 2021 **and** SOTM Jun 2021.
- **"Lusion idle render loop ~43 rAF ticks/sec"** — false: **48.3 ticks/sec** (54 ticks in 1.118s). Same order; the continuous-render finding holds.
- **"Lusion's third canvas is a full-size overlay"** — false: it measures 2163×1017 backing store at 0×0 CSS size, so it is not laid out at full size. Canvas count and the two named dimensions are exact.
...
- **"Lusion's SOTY 2023 peer-vote tally was 95/220"** — unsupported: single-source, not independently confirmed. The title itself is verified on the Sites-of-the-Year listing.
- **"The Aristide faces are Timmons NY (Matt Willey) and Jon Way Studio"** — unsupported: unprovable from the `t.woff2` / `jw.woff2` filenames. Weights 400 and 700 are verified; the names are not.
- **"Bruno's landmarks are Home, Options, Achievements, Circuit, Behind-the-scene, Three.js Journey, Devlogs, Source code, Musics"** — unverified: a carried-over list, never re-walked in the live world.

**Evidence base.** Every copy verdict above rests on raw-HTML grep plus tag-strip, or on the rendered DOM. Hero strings returning zero on literal grep do so because inline `<strong>` and SplitText spans split them; tag-stripping reassembles and they match. Two Aristide strings that circulate secondhand are absent from the static shell entirely.
