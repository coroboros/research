---
title: "Brutalist — Effect Palette, Page Recipe, Aliveness"
date: "2026-07-30"
author: "Coroboros"
...
# Brutalist — Effect Palette, Page Recipe, Aliveness

The brutalist line tops out around 7.5 on the Awwwards jury, and its mid-page stays alive by layering a scrubbed decor texture behind fire-once content reveals. Per-effect recipes read off awarded sites in the brutalist / neo-brutalist line, the ordered page scaffold that carries them, and what the line's highest scorers ship to keep the zone between hero and footer alive. Every recipe cites a named winner with its award, its date, and values read from its live CSS, its shipped JS, or a builder-authored case study. Parent reference: `../award-winning-websites-2025-2030.md` §1.2.

## Effect palette

### Corpus — the site roster

Award, date, score, credits, register, and how deeply each site was read.

1. **FlowFest 2025** — `https://www.flowfest.co.uk/`. Awwwards Site of the Day (SOTD) 29 Jul 2025, **7.36** + GSAP Site of the Week + Developer Award. Credits: Dennis Snellenberg, Osmo, Ilja van Eck, Isabel Edwards. Warm-illustrated register — butter/pink/orange, round buttons, chunky type. **Read:** live CSS + 982KB raw HTML + the full Slater bundle (`https://slater.app/14984/39049.js`, 31.6KB raw JS; `https://slater.app/14984/39050.css`) + `flowfest-2025.webflow.shared.css`.
2. **Eloy Benoffi** — `https://eloyb.design/`. Awwwards Honorable Mention (HM) 4 Jul 2025, no jury number published on an HM + GSAP Site of the Day + CSSDA Best UI / Best UX / Best Innovation. Glitch/terminal register — charcoal `#2B2C27`, Px437 VGA bitmap font. **Read:** live CSS + 1.46MB raw HTML with inline GSAP + `eloyb-design.webflow.shared.min.css` (58KB) + Codrops case study, 15 Oct 2025.
3. **Sui Overflow 2025** — `https://overflow.sui.io/`. Awwwards Site of the Day 15 Apr 2025, **7.48**. HOLOGRAPHIK + Ilja van Eck. Saturated/structural register — 2px hard borders, 0-radius controls, Sui-blue accent. **Read:** live CSS (2025 tokens) + 451KB raw HTML of the 2026 successor edition now served at the URL; structure and copy below are the successor's.
4. **Naked City Films** — `https://www.nakedcityfilms.com/`. Awwwards Site of the Day 23 Jan 2026, **7.34**, SavoirFaire / Tomas Kmet; Awwwards tags WebGL + GSAP. Industrial/editorial register — off-white on black, Haas Grotesk, WebGL scene. **Read:** live CSS — the origin now returns `403` — + Codrops case study, 19 Jan 2026.
5. **Treize Grammes** — `https://13g.fr`. Awwwards Honorable Mention 11 Oct 2024, Thomas Carré / 13G. Activation-concept brutalism — the fake-button-to-video morph. **Read:** 423KB raw HTML (strings, computed hexes, script srcs) + `13g.webflow.shared.min.css` + the production bundles `main.js` (10.7KB) and `loader.js` (1.7KB) + Codrops case study, 10 Oct 2024.
6. **Joffrey Spitzer** — Awwwards nominee, Astro + GSAP + Swup. Minimalist with a brutalist edge; borderline archetype, carried as **single-source / supporting** for its stepped preloader, Flip handoffs, and mask reveals. **Read:** Codrops case study only, 18 Feb 2026.
7. **DesignThinkers 2020** — `https://www.awwwards.com/sites/design-thinkers-2020`. Awwwards Honorable Mention. Inverted-color hover brutalism; **pre-window (2020), supporting** only.
8. **Amaterasu** — `https://www.awwwards.com/sites/amaterasu`. Site of the Day 14 Nov 2024, **7.54**; Awwwards tags Technology / 3D / Storytelling. Cited only to fix the archetype ceiling — not brutalist ("organic" is a reader's label, not an Awwwards tag).
...
Text link shifts color on hover with a short color-only transition — dim toward grey, or up toward the accent. Restraint over decoration.
...
- DesignThinkers 2020: inverted-color hover across the grid (observed, implementation unverified, **pre-window**).
...
- DesignThinkers (single-source, supporting).
...
- Treize Grammes (author-stated params). *Single-source* — and the Eloy read carries the identical numbers, so the two are one recipe with an unresolved site attribution, not two corroborating sites. The source read is on Eloy.
...
- Treize Grammes (author-stated). *Single-source* — the Eloy `tlWorkScroll` values are identical, and "Selected Work" is Eloy's `.work-header … .title-txt`, so treat the shipped-JS read as the attribution.
...
- Treize Grammes (author-stated). *Single-source* — the Eloy `tlHeroSVG` values are identical and the flower paths are Eloy's, so treat the shipped-JS read as the attribution.
...
Per-winner loader beats and handoffs are in "## Page recipe" → Loader in the page sequence.
...
**What generalizes.** Each site holds one physical metaphor (object / control / terminal / print sheet), gives every element class the interaction its *affordance* implies under that metaphor, and runs all of them on one easing family and one border/shadow/radius system. Variety across classes plus unity of metaphor and values is what these two SOTD winners do that a single repeated hover cannot.

### Anti-signals — absent from every brutalist winner examined
...
#### Shape C — The Stepped Reel *(single-source)*
...
#### Hero 1 — Type-as-image slab *(FlowFest, live-verified)*
...
| 5 | `.rainbow-sides__left/right` (4 arches) | draw from edges | ~3s each | `animateRainbow` |

Rows 1–4 are verbatim from the raw Slater JS: `-102`, `Expo.easeOut`, `stagger: -0.025`, `duration: 1`. Row 5 is the weak row — `rainbowsScrolltrigger()` exists in the JS and the arches are real, but the "4 arches / ~3s each" count and duration are unconfirmed in source. [`../surfaces/interaction-channels.md`](../surfaces/interaction-channels.md) carries the same figures inside a winner-verified row.

#### Hero 2 — Identity-tag terminal hero *(Eloy, live-verified)*
...
#### Hero 3 — Showreel-curtain hero *(Joffrey, single-source)*
...
#### FlowFest — chat-cloud typing loader *(live-verified, from `initLoader`)*
...
#### Joffrey — stepped counter + clip-path + Flip *(technique, exact params)*
...
#### Eloy — loader-into-navbar expansion *(technique)*
...
#### Naked City — scroll-driven welcome transition *(technique)*
...
**Absence is data, within scope.** Across the four sites whose intros were read, a zero-ceremony instant paint was not observed once; Sui Overflow's entry is unread on this axis and contested. The brutalist tell is the character of the intro — typed chat, `steps()` counter, container expansion — not its presence.

### Route transitions *(multi-page winners)*
...
|---|---|---|---|---|
| FlowFest | `lenis@1.3.1` | `gsap@3.13.0` | ScrollTrigger, CustomEase, DrawSVGPlugin, TextPlugin, Draggable registered at L4; SplitText + InertiaPlugin loaded but never registered. `@barba/core@2.10.3`, Slater bundle | `<script src>` + `slater.app/14984/39049.js` |
...
- **Rainbow arches drawn on scroll.** `animateRainbow('.rainbow-vertical__1', 'fromStart', …, { trigger: '.welcome', endTrigger: '.about__tile', scrub: 0 })` and `('.rainbow-vertical__3', …, { trigger: '.about__bottom', start: 'top 100%', end: 'bottom 50%', scrub: 0 })` — DrawSVG arches stroke in as the mid-page prose scrolls under them.
...
- **Clone field on the about/connect section.** `TRIGGER_DISTANCE = 200` px of cumulative pointer travel spawns `COPIES_PER_TRIGGER = 2` clones, uncapped, in `mix-blend-mode: difference`; `mouseleave` fades them at `opacity: 0, scale: 0.6, duration: 0.2, ease: "back.in(1.7)"`.
...
- **FlowFest "24-section scroll (live-verified)"** — false: the live HTML carries **9 `<section>` elements** and `class="section"` ×0. The 18–24 figure counts finer scroll beats, not sections; the funnel order survives, the count is observed.
- **FlowFest "near-instant first paint / no preloader ceremony"** — false, per its own `initLoader`: a full chat-cloud typing loader runs on first load, and no zero-intro winner exists in the line.
- **FlowFest stacked-card deck runs on InertiaPlugin** — false: the deck runs `Draggable.create(firstEl, { type: 'x', … })` with a hand-rolled `onDragEnd`, and the release throw is `gsap.to(…, { ease: 'elastic.out(1,0.75)' })`. InertiaPlugin ships in a script tag but is never registered — L4 registers ScrollTrigger, CustomEase, DrawSVGPlugin, TextPlugin, Draggable only.
- **Eloy `tlWorkCardReveal` stagger `{ each: 0.15, from: "random", ease: "power2.in" }`** — false: no such stagger exists in the card reveal. The three chained `fromTo` calls use `stagger amount 0.2 / 0.15 / 0.1` with stagger eases `power1.out` / `power1.inOut`, under main eases `power3.in` / `power3.inOut` / `power3.out`.
- **Eloy's clone-storm lives in the footer** — false: it is bound to `.about-section` under the inline comment `/*CONNECT POPUPS*/`, and the copies exit on that section's `mouseleave`. The word "footer" appears once in the entire HTML, unrelated to the clones.
- **Eloy's clone-storm duplicates one CTA up to a maximum of 200 copies** — false: `COPIES_PER_TRIGGER = 2` and there is no cap; the 200 is `TRIGGER_DISTANCE`, a pixel step on `mousemove`.
- **Treize Grammes jury score 8.69** — false: an Awwwards Honorable Mention page carries only community votes ("Votes 15/27"), no aggregate, no `aggregateRating` / `ratingValue`; every "8.69" in the HTML is a CSS `clamp()` or an SVG path coordinate. The 15 visible votes average ~8.77, and "jury" mislabels an HM.
- **Sui Overflow ships `lenis@1.1.13`** — false: the script tag reads `lenis@1.1.14`, with 11 references in the bundle.
- **Treize loads Barba** — false: no script tag, and Barba is absent from both production bundles (`main.js` 10.7KB imports only `loader.js`; `loader.js` 1.7KB references only gsap — neither can contain Barba core). Only vestigial `data-barba="wrapper|container|link"` and `data-barba-namespace="home"` attributes remain, alongside stale `localhost:3000` dev module tags.
- **Sui's "$500K+ in total prizes across core and sponsored tracks" is the on-screen copy** — false: that string is the `sr-only` span. The visible display copy reads "$500K+ in total prizes **and rewards** across core and **specialized** tracks"; the quoted string is the accessibility variant.
- **FlowFest 2025 carries a flat `#F3A20F` / `#F97028` palette** — unsupported by any measured read in this corpus: neither hex occurs in FlowFest's live CSS, which carries pink `#F489A3` and off-white `#FFFEFB` button fills with `#121212` off-black ink on borders and nav. The pair is parent-article-only ([`../award-winning-websites-2025-2030.md`](../award-winning-websites-2025-2030.md)).
