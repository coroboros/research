---
title: "Bento / Card — Effect Palette, Page Recipe, Aliveness"
date: "2026-07-30"
author: "Coroboros"
...
# Bento / Card — Effect Palette, Page Recipe, Aliveness

Bento is a *section* pattern — feature grid, product showcase — and no award-anchored winner opens on a card grid; the 2025 winners lean editorial rather than a saturated tile wall, the bento-fatigue correction visible in the work. Jury scores on the line run 7.62–8.05 overall. Effect palette, page recipe, and mid-page aliveness for the bento / card archetype. Parent reference: [Award-Winning Websites — 2025–2030 Reference](../award-winning-websites-2025-2030.md).

Whole-site bento award winners are rare, so the awarded evidence concentrates in a few hard-awarded whole sites plus feature-grid sections inside design-canonical product pages. Evidence tags: `(winner-verified)` = computed styles, CSS, JS, or DOM read off the running page. `(shipped)` / `(design-canonical)` = observed or documented, exact parameters unverified. `(technique)` = a documented method, typically Codrops. `(single-source)` = one site only.
...
Awwwards Honorable Mention panels grow as jurors keep adding notes, so any single decimal is a moving snapshot. Figures above read from the rendered jury grid on 2026-07-13, per-juror cells summed by hand.

Anime.js's Awwwards entry lists its palette as `#252423` / `#DAD5D0`.

Technique references, never cited as awarded: Codrops GSAP breakdowns (product-grid preview, elastic grid scroll, sticky grid scroll), Cuberto (live-read for the easing vocabulary `cubic-bezier(0.16,1,0.3,1)`, `cubic-bezier(0.19,1,0.22,1)`, and spring `cubic-bezier(0.34,5.56,0.64,1)`), the documented Vercel border-hover recreation.

### The score reality

Award winners cap at **7.62–8.05 overall jury**; the corpus's highest whole-site score is Lusion's 8.25 ([`experimental.md`](./experimental.md)). The standout numbers on this line are always sub-scores — Anime.js Animations/Transitions 9.00, Endex Usability 8.27 and Content 8.07. This is the bento-fatigue ceiling, confirmed in the scores.
...
- **Trailing-icon shift.** The label holds while an arrow or chevron nudges ~4px on the x-axis over `0.2s`. Pairs with either style above. `(single-source, supporting)`.
...
- **Border-glow bloom.** A blurred accent gradient in a pseudo-element behind the card fades in on hover (`opacity 0→1`), reading as a soft under-glow rather than a hard shadow. The accent is the tile's own OKLCH token, not a global grey. *When:* dark grounds (`--bg` around `#0A0A0F`) where glow reads. Supabase plus Linear spotlight-card lineage, both documented. ≥2.
...
- **Tilt (perspective follow).** Pointer position drives a small `rotateX` / `rotateY` (≤6–8°) via motion values outside the React render cycle. Named in the standard bento affordance set alongside spotlight and border glow, but amplitude was never verified on a live winner. `(observed, implementation unverified)`. Single hero tile only — tilting every tile is noise.

**The rule these encode:** pick one hover affordance per grid — border-shine *or* spotlight *or* glow *or* live-demo — and let the tiles differ by content, not by each inventing its own motion.

### Nav items and the nav bar

The AI-default: links get the pale-fill pill on hover; the bar turns frosted glass on scroll **and hangs a contrasting-colored `border-bottom`**. No winner examined hangs a differently-colored border under the nav — it is a template tell.
...
**Vercel — "one material idea: light catching an edge."**

| Element | Interaction | Verification |
|---|---|---|
| Button | Solid pill (`radius 100px`), token-step or invert on hover, crisp ~0.15s | documented |
| Text link | Underline or color shift | documented |
| Card | Conic-gradient border-shine tracking `--x` / `--y`; spotlight expand plus reveal | documented |
| Nav bar | Transparent at top → frosted blur plus same-family hairline on scroll | documented |
| Cursor | Native; the pointer feeds the border-shine | documented |

A single *material* recurs at every scale — a light streak on an edge. It rides the card border under the cursor and glints on the CTA, and the system is monochrome Geist precision (Ink `#171717`, tight `-2.4px` tracking, mono eyebrows, stacked shadows over heavy drops) so the light motif is the only warm event. Button, card, and nav differ in mechanism but obey the same physics, tuned to each surface.

**The composition lesson.** Cohesion comes from a shared through-line — Anime.js: *motion*; Vercel: *edge-light* — not from repeating one gesture. Each element class carries its own mechanism, all of them tied to one idea. One pale-fill hover on every button plus one frosted-hairline nav everywhere has no through-line; it is sameness mistaken for consistency.
...
- **One universal card hover** (`translateY(-4px)` plus `scale(1.02)` plus grey drop-shadow) applied to every tile.
...
**Fingerprint.** One dark ground (`#252423`) held for the whole scroll. The page opens on a left-copy / right-demo hero, drops into one big central capability grid, then hands off to a run of full-viewport panels — each `height: 100lvh`, its copy `position: fixed; opacity: 0` and cross-fading as you scroll, while a single persistent live demo runs and proves the heading. The bento *grid* of asymmetric tiles lives in the toolbox and modules sections; the feature gallery between them is a pinned reel, not a scrollable tile wall. It closes on a bundle-size stat, a sponsor wall, a get-started CTA, and a newsletter footer, and assembles itself with no loader.

**Ordered skeleton** — verbatim `id` / `data-label` from the DOM, intensity in brackets. Intensity `[n]` is a comparative 1–10 estimate of attention load (defined in `../winners/composition-chains.md`).
...
**H1 — Left-copy / right-demo split (Anime.js, winner-verified).**
...
- The heading sits top-left at `#intro .home-section-text h2{font-size:var(--text-xxxxl)}` — the largest step, three lines split with `<br>` ("All-in-one / animation / engine") and a `<span class="animation-engine red-dot">.</span>` as the terminal period.
...
**H1 — Product-render tagline stack (Apple, shipped).** H1 is the product name (`<h1 class="header-eyebrow …">MacBook Air</h1>`), a two-to-four-word poetic tagline directly under it, then a spec line. Media is a product render on an unbroken studio gradient; the bento is the first content beat after the fold.
...
- **Looping demo machines.** Demos default to `loop:true` (26 occurrences in the bundle, on objects keyed by `id`, e.g. `id: "draggable-meshes-rotate"`); when `.feature-section.is-in-view`, `pointer-events:auto` lets you grab, drag, and throw the Draggable panel and re-run it. Perpetual and interactive.
...
- **Hero idle loop.** `<span class="animation-engine red-dot">.</span>` with `#intro .home-section-text .red-dot .char{opacity:1!important}` — the headline is split into `.char` spans and the period is a char held visible while it pulses colour and scale forever.
...
**Vercel** `(winner-verified, corroboration)`. The alive tiles are CSS-transition widgets, not a motion library: the install-command tile animates width on `duration-[400ms] ease-[cubic-bezier(0.32,0.72,0,1)]` with `motion-safe:animate-command-fade-in`; nav links brighten muted→foreground on `transition-colors duration-100`. No Lenis, GSAP, Framer Motion, or Locomotive anywhere in the served markup.

### Hover on text specifically — the tier ships almost none
...
### Smooth scroll — absent across the entire tier

None of the three award winners smooth the wheel. No Lenis, Locomotive, or `@studio-freight` string appears in any served bundle.
...
- **Endex** — no Lenis, GSAP, Framer Motion, or Locomotive anywhere. An adversarial grep flagged `framer` ×11 and `gsap` ×2, all false positives (`ComponentFrameRoot` and `forceFrameRate` matching "framer"; a PostHog `flagsApiHost` matching "gsap"); the package tell `framer-motion` counts zero.
...
- **No universal card lift.** Meridian's group-hover scale is `1.02` on select cards only; Anime.js has no card lift at all; Endex uses group-hover colour and translate on specific cards, never a blanket lift.
...
- **"The Docs nav item gets the red `:before` pill"** — false: the pill belongs to the **Sponsor** button, `#site-menu sponsor-button .main-nav-link:before`. No `.docs-link` styling exists; a live screenshot shows Sponsor carrying the pill while Docs is muted text. The split hero re-confirmed live, heading measured at 432px from the left against an animated circular SVG demo on the right.
- **"Footer links fill red on hover — `a:hover{color:var(--hex-red-1);background:var(--hex-red-6)}`"** — false: that rule is scoped to `.links-list li sponsor-button a:hover`. General footer links use `--hex-current-1` on `--hex-current-7`, resolving to `--hex-fg-1` / `--hex-fg-7` inside the footer's `data-color="fg"` scope — a monochrome fill. The instant timing and the icon nudge hold.
- **"The Anime.js hero heading is `--text-xxl`"** — false: it is `--text-xxxxl`. `--text-xxl` is the toolbox and feature-panel heading size; copying it sets the hero two steps too small.
- **"Meridian scored 7.89 overall"** — false: **8.05**. Fifteen juror totals — 7.40, 6.60, 8.00, 8.00, 8.00, 8.40, 6.90, 9.40, 8.20, 7.40, 9.40, 7.10, 9.20, 9.00, 7.70 — sum to 120.70 over 15; an independent cross-read corroborates at 8.04. Sub-scores: Design 8.20, Usability 7.93, Creativity 8.00, Content 7.87. Still the line's highest overall, still below 8.5.
- **"Endex scored 7.79 overall"** — false: **7.91**. The earlier read missed a 10/10/10/10 juror.
- **"Endex Content 8.27, Design 8.13, Usability 8.07, Creativity 7.60"** — false on every figure. Live column means over fifteen jurors: Design **7.93**, Usability **8.27**, Creativity **7.27**, Content **8.07**. **8.27 is the Usability column, not Content.** The editorial-content lesson holds — Content 8.07 is Endex's second-highest column — but the number and its label were wrong.
- **"Ceiling 7.62–7.89"** — false: **7.62–8.05**, following from Meridian. The "no 8.5+ whole-site" finding stands.
- **"Looping demos are keyed by `name:`"** — false: the source object key is `id:`.
- **"The `.scroll-bar` track is 1px `dotted`"** — unsupported as literal CSS: it is a repeating 1px `linear-gradient`.
- **"Meridian ships 32 `transition:` shorthands"** — false: 32 counts all `transition*` declarations; the shorthands number 24.
