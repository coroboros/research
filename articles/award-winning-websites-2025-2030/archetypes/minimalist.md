---
title: "Minimalist — Effect Palette, Page Recipe, Aliveness"
date: "2026-07-30"
author: "Coroboros"
...
# Minimalist — Effect Palette, Page Recipe, Aliveness

Verified minimalist winners cluster at 7.2–7.9 on the Awwwards jury, and their mid-page life comes from one scroll-welded decor channel rather than from text hover. What awarded minimalist sites actually ship, effect by effect, page part by page part, and in the prose zone between hero and footer. Parent reference — typography, palette, use cases, canonical winner: [Award-Winning Websites — 2025–2030 Reference](../award-winning-websites-2025-2030.md). Evidence tags: **(winner-verified)** the live CSS or DOM was read; **(Codrops-verified)** the site author's own case study; **(shipped)** observed live or in award media, implementation not read; **(technique)** documented method; **(single-source)** one site or one source carries it.

## Effect palette

### Corpus

| Site | URL | Award | Jury | Stack |
|---|---|---|---|---|
| **Terminal Industries** | `terminal-industries.com` | Awwwards Site of the Day (SOTD) 3 Sep 2025 + Site of the Month Sep 2025 + Developer Award; CSS Design Awards Website of the Day Aug 2025 (final 8.42) | **7.68** design, **7.89** dev | Nuxt/Vue, Lenis |
| **Gabriel Contassot** | `gabrielcontassot.com` | Awwwards SOTD 14 Apr 2024 + CSS Design Awards | **7.34** design, **7.63** dev | Astro, Lenis 1.0.42, GSAP + ScrollTrigger |
| **Stefan Vitasović — Portfolio25** | `stefanvitasovic.dev` | Awwwards SOTD 20 Sep 2025 + Developer Award (8.80 animations — *contested attribution*) | **7.25** — Design 7.31 / Usability 6.94 / Creativity 7.51 / Content 7.38 | Next.js, Framer Motion, R3F |
| **Treize Grammes (13G.)** | `13g.fr` | Awwwards Honorable Mention (HM) 11 Oct 2024 (8.52 community) | HM tier, 6.5+ | Webflow, Lenis 1.1.13, GSAP 3.12.5 + ScrollTrigger, three.js r128, Swiper 11 |
...
**Stripe** (`stripe.com`, "HDS" design system) is reference-tier — CSS-verified for button and nav-surface triangulation, not itself an award page.

Three cautions on the roster. Treize's 8.52 is a community figure recorded off the award page, not a jury score; the page publishes no aggregate, only per-voter community votes — 15 visible, averaging ~8.77 (`brutalist.md`, which reads the same site). Gabriel's SOTD date on the live award page is **14 Apr 2024**. And `awwwards.com/sites/portfolio-25-1` is a **different site** — Roman Jean-Elie's "Portfolio '25", 7.21, tagged 3D / GSAP / Three.js / Next.js, SOTD 2 Nov 2025 — not Stefan's.

### The score reality

Jury scores cluster **7.2–7.9** across verified minimalist winners; the corpus's highest whole-site score is Lusion's 8.25 ([`experimental.md`](./experimental.md)). The cause is the weighting — Design 40% / Usability 30% / **Creativity 20%** / Content 10%, with Honorable Mention at 6.5+. Creativity rewards spectacle; restraint caps the ceiling. Sites scoring 8.5+ are near-exclusively immersive, WebGL and experimental. So the tier to study is the line's own highest scorers, the 7.25–7.68 SOTD winners in the corpus above. The universal negative is inductive over that sample, not exhaustive.
...
**R1 — Directional token-wipe inversion (primary CTA).** A pseudo-element the color of the dark token sits translated below the button inside an `overflow:hidden` pill and slides up on hover, inverting the token pair. Terminal's lime button (`#abff02` bg / `#052424` text) becomes `#052424` bg / `#abff02` text. **(winner-verified)**
...
**R2 — Full-token background shift, ink held (primary CTA, flat systems).** No wipe; hover swaps `background-color` to a dedicated hover *shade of the same brand hue* (`--hds-color-button-primary-bgHover`), text unchanged. The secondary variant shifts both background and text tokens. A true darker or lighter step of the accent, not a transparency wash. **Where it fits.** Airy Swiss-grid SaaS where a moving fill would read as noise. **(winner-verified, reference-tier)** — Stripe `.hds-button:hover`, `.hds-button--secondary:hover`. Second corroboration: Terminal's `.drawer-cta-button:hover` (white→lime, or lime→dark-green token swap, `.2s ease`). **(winner-verified)**

**R3 — Underline draw under the label (link and CTA sub-mark).** A 1px `:after` bar, `transform:scaleX(0)`, drawn to `scaleX(1)` with the origin flipping on hover. Terminal runs it at two speeds, and the two are not interchangeable:
...
**R8 — Nav-bar surface: float transparent, or frost translucent.** Two verified treatments, zero colored border-bottoms.
...
**M5 — Scroll-linked parallax by inverse scale, small offset.** Images scale inversely to scroll progress — `scale(1.2 + track.value * -0.2)` — while a masked inset opens; project media is synced to a camera or scroll value with a slight offset for depth. Amplitude is ±0.2 scale, never a 50%-translate parallax. **(Codrops-verified)** — Gabriel Contassot, Rogier de Boevé ("synced… with a slight offset to create a parallax effect").

**M6 — Section and route transition is a short opacity crossfade.** Stefan: `duration:0.5, ease:easeQuadInOut` via AnimatePresence. Gabriel: a full-screen color overlay fading at `slow.in` easing, `0.8s`. No slide-the-whole-page theatrics. **(Codrops-verified)**

### Text effects

The AI default is a typewriter effect, or one headline fading up as a block. Either overplays or underplays.

**T1 — Per-char masked reveal, indexed stagger (signature).** Each glyph in its own `overflow:clip` wrapper with a kerning fix, translated in on an index-scaled stagger under expo easing:
...
| Ghost/tertiary btn | 5% dark tint fills, text held (R6) | lime family · fast `.2s` |
| Nav item | label → lime, a 5px lime **dot grows** below (R7) | lime resolve · `1s cubic-bezier(.075,.82,.165,1)` · origin = center-below |
| Logo | color → lime, nothing moves | lime resolve · `.3s` |
| Content link | text opacity→1, underline **draws** L→R, arrow **nudges** up-right 2px (R3+R5) | lime · underline `.3s` · origin = left |
...
**Gabriel Contassot — a near-monochrome system that still differentiates class.** **(winner-verified + Codrops-verified)**

| Element class | Geometry | Shared invariants |
|---|---|---|
| Nav / menu labels | **scramble/decode** text on enter, hover-locked (T2) | mono type · indexed timing |
| Project images | `clip-path:inset()` reveal + inverse-scale parallax `scale(1.2+track*-0.2)` (M3/M5) | scroll-linked · small amplitude |
| Inline links | underline draw + arrow direction cue | restrained, editorial |
| Oversized label | `mix-blend-mode:difference` inverts over scrolling content (C2) | no custom cursor |
| Route change | full-screen overlay fade, `slow.in 0.8s` (M6) | opacity-only |
| Intro | accelerating 1→100 counter, 2.8s (L2) | numeric, no spinner |
...
**The composition rule.** Assign each element class its own recipe, then lock `--accent`, one easing family (expo for content, sine for color fades), and a single origin logic across all of them. If two classes share a geometry, one is redundant — differentiate or merge. The primary CTA and an inline link never resolve identically.

### Anti-signals — effect level

Absent from every winner examined.

- **A universal pale-tint fill-sweep on every button.** The 5–10% accent wash exists only on Terminal's ghost button. Every examined primary CTA moves the full token — invert (R1) or shift to a real hover shade (R2).
- **A colored `border-bottom` under a solid nav bar on scroll.** Zero winners. Nav bars float transparent or frost translucent (`rgba(...,.8)` + `blur`). No contrasting hairline hung under the bar.
- **The same underline slide on buttons, links and nav.** Winners split it: draw on inline links (R3), dot on nav items (R7), wipe or shift on buttons (R1/R2).
- **A custom circle-follower or magnetic-blob cursor.** Three sites keep the default cursor; the only blend trick lives on a text element.
- **One global `fade-up 20px, .6s ease` on every section.** Reveals are masked under expo with indexed per-element timing. Treize's Webflow `fade-up=""` is the corpus's weakest motion and marks the line's floor, not its bar.
- **Tailwind's `cubic-bezier(.4,0,.2,1) .15s` as the site's motion identity.** Fine for incidental UI — Terminal quarantines it to 6 utility overlays, Gabriel's sheet carries it on an unused `.transition` utility. Leaving everything on the framework default reads as no motion decision.
- **`Inter` as the display face.** Every winner picked a face with character: Suisse Int'l (Terminal), HelveticaNow Display (Stefan), Monument + Söhne (Gabriel), Neue Haas Grotesk (Rogier).

## Page recipe

Section count runs 6–14; page length 4–14 viewport-heights. Two structural families cover four of five winners; the fifth is a single-canvas outlier.
...
Ordered skeleton, Terminal, from the live DOM — the funnel order and the climax are **(winner-verified)**, the exact 9-count is approximate. Intensity `n/10` is a comparative 1–10 estimate of attention load (defined in `../winners/composition-chains.md`):
...
**H3 — Editorial promise line (Treize Grammes).** **(winner-verified copy)** A short second-person imperative in `<h1>` — "Réveillez votre croissance", where the H1 opens "Réveillez" and "votre croissance" completes it — over a one-line subhead: "Agence de design de marque 100% créative : branding, stratégie, site et web app,  identités visuelles." (verbatim, including the double space; one DOM instance carries the singular typo "identité visuelles"). CTA present and warm: "Programmer une visio". The expressive edge — reveals are Webflow `fade-up=""`, not masked.

### Loader and intro

Three verified forms. Never a spinner, never a blocking brand-color splash.

**L1 — Split-curtain plus recoloring counter (Terminal Industries).** **(winner-verified)**
...
**L2 — Bare accelerating counter (Gabriel Contassot).** **(Codrops-verified)** A 1→100 numeric counter whose `setInterval` fires at progressively shorter intervals, `values = [1,2,…,100]` with `splitDuration = totalDuration/values.length` hardcoded per index, total ~2.8s, values positioned with `translateX()`. No curtain, no logo theatre; it retracts straight to the text-only name card.

**L3 — Instant paint, reveals are the intro (Treize; the sanctioned default).** **(shipped)** Webflow `fade-up=""` sections paint immediately and the scroll reveals *are* the intro. Legitimate when the reveals carry enough choreography — but Treize's plain fade-up is the corpus's weakest motion and marks the line's floor.

### Route transitions

Multi-page winners only. The transition language rhymes with the loader curtain.

- **Terminal** — in-page element reveals use the Vue `reveal-y` transition (M4). Route changes reuse `.app-loader` with the dark `rgba(0,0,0,0.7)` overlay variant, a curtain cover that rhymes with the intro curtain. **(winner-verified markup; full sequence shipped)**
...
- **Stefan** — SPA route changes scored **8.80** on animations for the Developer Award; "Transitions / Motion / Filters and Effects" are the named strengths. Exact mechanics are JS-gated, not read. **(shipped)** The 8.80 is a *contested attribution*: the identical figure is carried for Terminal Industries in [`../winners/site-of-the-year-contenders.md`](../winners/site-of-the-year-contenders.md) and for Truekind in [`editorial.md`](./editorial.md), so at most one of the three readings is the site's own — see [`../foundations/interaction-architecture.md`](../foundations/interaction-architecture.md).
...
- **Gabriel.** An oversized label crossing a light/dark section boundary under `mix-blend-mode:difference`, plus scramble/decode text with hardcoded per-index timing and inverse-scale masked figure reveals — `scale(${1.2 + this.track.value*-0.2})` as an `inset()` mask opens. **(Codrops-verified; scramble timings single-source; the blend itself read live in the Astro CSS)**
- **Stefan.** The hero name reveal — masked segments driven by Framer Motion on an indexed stagger, `duration: 1.25 + index * 0.025`, `easeExpOut`, used once. **(Codrops-verified)**
...
- **No hero carousel or slider.** Statements sequence in place (Terminal's `title-sequence`); they do not slide as a gallery.
- **No hero stacks two CTAs.** One CTA (Terminal) or none (Gabriel, Stefan) — the scroll cue can be the only affordance.
- **The `<h1>` is not assumed to live in the hero.** Terminal's hero is an `<h2>` sequence and the `<h1>` is the footer statement. Committing "hero = h1" by reflex misreads the line.
- **No spinner and no blocking brand-color splash.** Loaders are split-curtain plus counter, bare counter, or instant paint — always retracting to a composed fold.
- **No second accent visible.** Two to three colors carry the page and the accent appears once per viewport — Terminal lime once, Gabriel none against pure `#000`. Terminal does define `--c-orange:#fb6b3c` as an unspent token; the rule is about what a viewport shows, not what the palette declares.
- **No mixed-grade imagery.** One photographic grade or one monochrome across the entire page.
...
**Terminal Industries — 7.68 SOTD (anchor, live CSS + live DOM).** Nuxt/Vue + Lenis, ~22 viewports (`scrollHeight 14717`, `vh 676`). The mid-page argument walks logo strip → "Powering the yards behind the  brands you know" → "Imagine the yard as an intelligent bridge…" → "Yard Operating System. YOS™" → "Built by logistics leaders…" → executive quote → contact. Live bundles, filenames stable across both reads: `entry.Bdya6FOo.css`, `PaddedCounter.D_9jWspQ.css`, `get-frames.BEFbNTY1.css`, `NewsSection.CgFcU66P.css`, `BackgroundCanvas.BfIfKjEO.css`, `NotchSection.CmaDBuu9.css`. Mid-page mechanics, all live-CSS-verified:
...
The precise reading: the mid-page feels alive on re-scroll **because the decor channel is positional, not triggered** — it has no "already fired" state to get stuck in. Merely-good builds fire everything once on an IntersectionObserver and the page is inert on the second pass. The fix is not to make reveals replay, which reads cheap; it is to add one positional or scrubbed decor channel that has no fired state. Stated limitation: the live pages could not be driven to catch an in-flight replay — Terminal's Nuxt + Lenis setup rejects synthetic `scrollTo` and wheel events under automation — so the re-fire determination is mechanism-grounded, not motion-captured.
...
### Anti-signals — mid-page

- **No per-char hover, no weight or optical-size shift on hover, no color-sweep along the glyphs** on prose or headings. Text hover is strike-through or plain color-to-accent on links only.
- **No hover on body prose or standalone headings.** The one heading hover is a card title, and it is `text-shadow`, not a play.
- **No content reveal that replays on every scroll pass.** Replaying content reads cheap; only decor is welded to scroll.
- **No decorative ambient loop covering the mid-page.** About one quiet channel maximum, otherwise stillness between inputs.

## Refuted

- **"Stefan's hero subhead reads 'Crafting web experiences from Stockholm, Sweden. Awwwards Jury Member since 2020.'"** — false, and invented: the string is unfindable in the live DOM. The verified `textContent` is the longer 14islands/MSc sentence recorded under Copy voice.
- **"Terminal's `reveal-y` is `opacity` + `translateY(.5rem)` at `.25s ease`"** — false on both values: the live rule is `transition:opacity .6s cubic-bezier(.39,.575,.565,1),transform 1.2s cubic-bezier(.19,1,.22,1)` from `translate3d(0,100%,0)`. `.5rem` exists only on one scoped `[data-v-61c0a6e5]` leave-to variant.
- **"Tailwind's `cubic-bezier(.4,0,.2,1)` appears ×7 in Terminal's `entry.css`"** — false: ×6. The two-register easing grammar around it (×9 / ×9) holds.
- **"The line's voice formula refuses exclamation marks"** — false: Treize ships "Activez votre marque !" live.
- **"Stefan's per-char reveal wraps each glyph in its own `overflow:clip` wrapper"** — false, mis-attributed: Codrops describes "masked segments with `left` positioning and `x` transforms, not individual character wrappers with overflow clipping". The wrapper scaffold is Terminal's. The stagger `duration: 1.25 + index * 0.025` and `easeExpOut` stand, driven by Framer Motion, not GSAP.
- **"The `#ededed` `50svh` loader halves are `.overlay` panels"** — false: they are `.top-mask` and `.bottom`. `.overlay` is a separate full-height tint.
- **"`.footer-title` is the page's `<h1>`"** — false: it is a `div`. The `<h1>` is a separate wrapping element carrying the same text.
- **"Terminal ships no second accent"** — false as a palette claim: the palette declares `--c-orange:#fb6b3c` alongside `--c-lime`. True only of visible per-viewport usage.
- **"Gabriel ships no smooth-scroll library (winner-verified absent)"** — false: Lenis 1.0.42 is bundled in `hoisted.By4ZZI4s.js` and active at runtime. The absence was declared from the CSS and the `<script src>` list without opening the 558KB bundle — an absence claim about a library needs the bundle searched.
- **"Gabriel's motion is hand-rolled JS, no GSAP"** — false: `gsap.registerPlugin(...)`, `_gsap`, `GreenSock` and `ScrollTrigger` are all in the bundle, module-scoped rather than on `window`.
- **"Gabriel ships zero `transition` declarations"** — partly false: zero `:hover` rules holds, but the sheet carries Tailwind's `.transition` utility ×6, applied to nothing.
- **"The gallery-index register can win without Lenis"** — unsupported: with Gabriel corrected, 3 of 4 sample sites run Lenis and the case rests on Stefan alone.
