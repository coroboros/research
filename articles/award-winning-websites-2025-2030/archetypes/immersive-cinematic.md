---
title: "Immersive Cinematic — Effect Palette, Page Recipe, Aliveness"
date: "2026-07-30"
author: "Coroboros"
tags: ["web-design", "awwwards", "immersive-cinematic", "webgl", "gsap", "scroll-animation", "rive", "effect-palette", "page-recipe"]
sources:
  ...
  - "https://www.cssdesignawards.com/woty/lusion-v3/44311/"
...
|---|---|---|---|---|---|
| **Lando Norris** | `landonorris.com` | Awwwards Site of the Day (SOTD) 17 Nov 2025 (8.18); Site of the Year (SOTY) 2025 per the OFF+BRAND case study, unverified | **8.18** (8.12 / 7.9 / 8.71 / 8.18) | CSS + JS bundle + rendered DOM + screenshots | Webflow + GSAP + Lenis + Rive |
| **Siena Film Foundation** | `siena.film` | Awwwards SOTD Mar 18 2025 + **Site of the Month (SOTM) March 2025** + Developer Award; Niccolò Miranda | **7.9** (7.99 / 7.61 / 8.13 / 8.00) | CSS + JS bundle + rendered DOM + screenshots | Webflow shell + Vercel/Next overlay, Lenis, GSAP SplitText |
| **Lusion v3** | `lusion.co` | Awwwards SOTD Oct 2 2023 + Developer Award **8.41** (Animations/Transitions **10.0**); FWA + CSSDA Site of the Year 2023 | **8.25** (8.26 / 7.95 / 8.65 / 8.26) | CSS + JS bundle + rendered DOM | Astro + WebGL (OGL inferred) + custom lerp |
| **Messenger** | `messenger.abeto.co` | Awwwards SOTD Nov 10 2025 + Developer Award **8.21**; abeto | **7.92** (8.04 / 7.46 / 8.23 / 8.15) | score-verified, media-only | Three.js / WebGL / WebSockets |
...
| _Into the Storm_ | `airforce.com/intothestorm` | Awwwards SOTD Mar 23 2021 — out of window | **8.15** (8.24 / 7.73 / **8.5** / 8.35) | score only | — |

Behavioral and named-case evidence, no live CSS read: **Oryzo** (Lusion) — Awwwards SOTM April 2026 + Developer Award, camera-through-Z-depth scroll. **Bruno Simon portfolio** — Awwwards SOTM Jan 2026, drivable 3D world (Three.js + Cannon.js). **Cartier Watches & Wonders** (Immersive Garden) — Awwwards SOTD + CSSDA, six 3D rooms, Web Audio score, GSAP + Lenis. **Hubtown** (Unseen Studio) — Awwwards SOTD June 2026, cursor-reveal of geometry and lighting. **Telescope / Silent House / Sandy Shore** (Adrien Vanderpotte) — Awwwards SOTD, focal-zoom scroll and seamless scene transitions. **Obys Agency, Typography Principles** — Awwwards SOTD, Studio of the Year 2023, kinetic type. **Shopify Editions** — Awwwards SOTD, scroll-staged particle-dispersing type. **Explore Primland** — scroll-driven terrain flythrough. **Terminal Industries** — scroll shifts 3D visuals into wireframe.
...
- **Inner scale 1.1** — the image scales to `1.1` inside a fixed frame that clips the overflow. *Verified, two sites:* Lando `.helmet-grid-item-w:hover .helmet-grid-item-img-helmet { transform: scale(1.1) }`; Siena `.previousnext-item:hover .full-img-w { transform: scale(1.1) }`. Note the amplitude — **10%, not 3%**. A `1.03` scale is imperceptible and reads as dead; a grep for `scale(1.02)` and `scale(1.03)` returns **zero hits** on both bundles. **Where it fits.** The default media hover.
- **Clip-path ellipse uncover** — the media is revealed or hidden by a top-anchored elliptical mask that grows. *Verified, Lando-signature:* `[data-helmet-item]:hover .reveal-img { clip-path: ellipse(100% 120% at 50% 0%) }`; `.part-i-video-w:hover { clip-path: ellipse(80% 50% at 50% 50%) }` (single-source for this hover trigger). The `ellipse(100% 120% at 50% 0)` → `ellipse(100% 0% at 50% 0)` pair is what recurs across Lando's scroll reveals — every member of that family is anchored at `50% 0` or `50% 20%`, so the `50% 50%` hover anchor sits outside it — making the top-anchored form the whole-site motif rather than a one-off (verified end-state family in §Mid-page aliveness, Channel C). **Where it fits.** Reveals carrying brand shape themselves — a visor curve, a lens.
...
- **Transparent, adaptive, chrome-free** — the nav is `position: fixed`, `background: transparent`, no `backdrop-filter`, `border-bottom: 0 none`. Its text and icon color transitions (Lando: `color .75s cubic-bezier(0.65,0.05,0,1)`, the site's `--cubic-default`/`--duration-default` pair) to stay legible over whatever section scrolls under it. *Verified, two sites:* Lando `.css-nav` (transparent, no backdrop, no border, cream text adapting to dark and light sections); Siena `.nav-w` (fixed, transparent, no backdrop, no border). An adversarial grep returns `backdrop-filter` = **0** and nav `border-bottom` = **0** on both bundles. **Where it fits.** The archetype's default nav; a solid nav bar belongs to the corporate/SaaS archetypes.
...
**AI-default cliché.** A centered spinner, or a bare `0–100%` counter on a blank screen, hard-cutting to the page the instant it hits 100 — no continuity between the loader and the first frame.
...
- **Nav bar** — transparent, no backdrop, no border. **Cursor** — OS default.
...
### Anti-signals — element level, absent from every winner examined
...
#### Shape A — The Portrait Procession (Lando Norris) — (winner-verified section order; the pinned horizontal track is shipped / observed)

**Fingerprint.** A corner-locked wordmark over a full-bleed frontal portrait on a light canvas, then a long vertical procession of full-viewport chapters — each a facet of the person (on-track, off-track, honours, partners, socials) — with one horizontal-scroll pinned interlude, closing on a dark inverted portrait footer carrying an oversized split-color valediction. The palette flips light→dark exactly once, at the close, bookending the hero. Rest beats are short bridge sections between the full-height chapters. For athlete, personality, and founder portfolios with many facets and one face.

**Ordered skeleton** — class names and heights read element-by-element from the DOM. Intensity `n/10` is a comparative 1–10 estimate of attention load (defined in `../winners/composition-chains.md`):
...
9. `s is-lando-exe` — "World Drivers' Champion" honour · proof · 7/10
...
#### Shape B — The Gated Index (Siena Film Foundation) — (winner-verified)
...
#### Shape C — The Studio Manifesto → Project Reel (Lusion v3) — (winner-verified)
...
#### Shape D — The Single-Scene World (Messenger, Active Theory) — (technique / observed)
...
#### H1 — Corner-lockup portrait (Lando Norris) — (winner-verified structure, shipped composition)
...
- H1 text "Lando Norris"; subhead H2 raw `textContent` **"2025 Mclaren Formula 1 Driver"** — note the lowercase "c", while `document.title` separately reads "2025 McLaren Formula 1 Driver — Lando Norris". Both strings are verbatim from the DOM.
...
#### H2 — Statement-over-canvas (Lusion v3, Active Theory) — (winner-verified copy)
...
#### H3 — Costumed splash gate (Siena Film Foundation) — (winner-verified copy, shipped composition)

The gate *is* the hero: an oversized "SIENA" vintage high-contrast serif wordmark centered on pure black, "FILM FOUNDATION" letterspaced beneath, and a single ticket-stub "ENTER →" affordance with a perforated dashed divider (shipped, screenshot). It is its own fixed panel rather than a scrim over the reel — `.preloader-w{position:fixed; inset:0; z-index:99999999; background-color:var(--black); display:none}` with `.show{display:flex}`, carrying a `.preloader-video{height:50svh}` (`100svh` on mobile) and a `.preloader-btn{transform:translateY(200%); font-family:Neue Brucke}` that slides up into place ([`../surfaces/preloaders.md`](../surfaces/preloaders.md)). `<body>` computes `background: rgb(0,0,0)`, `color: rgb(250,247,239)` = `#FAF7EF`, `font-family: "NB International", Arial, sans-serif`; the display wordmark is a vintage serif. The gate node's isolated `textContent`, excluding style and script children, is exactly `"SIENAADMIT ONE004"` — and notably **not** doubled, unlike the film titles.

### Loader in the page sequence

Two hand-off families, plus one absence pattern.

- **Auto-retracting object-assembly curtain (Lando).** A Rive/GSAP preloader builds the signature object while assets stream, then opens into the live hero via the top-anchored `ellipse(… at 50% 0)` hand-off mask so there is no cut (observed, implementation unverified). The "Load Norris" pun labels the wait. Verified negative: no `[class*=load]`, `[class*=preload]`, or `[class*=curtain]` node survives post-load — the curtain unmounts rather than hiding. The negative does **not** extend to `[class*="intro"]`, which still matches surviving content nodes (`text-cta-short-intro`, `callout-socials-intro-w`).
...
- **Lando: a Rive idle layer that never freezes.** The runtime is confirmed — `riveAllLoaded` appears ×11 and the constructor is instantiated through a minified alias, `new i({src…,enableRiveAssetCDN…})`, inside `lando-by.js`. The live build carries **17** `data-rive-artboard` mount points (3 matching the literal `canvas[data-rive]` selector, 5 unique `.riv` files) alongside the 21 page-wide canvases. Ambient audio plus a live status card layer over them. These counts drift with an actively patched build, so they are **lower bounds rather than fixed integers**.
...
- Pattern: **the middle is a list of rows, and every row answers the cursor.** That is where the missing aliveness actually lives — not in animated prose, in interactive indexes.
...
- **Reversible decor** — Lando runs **29** concurrent `scrub:` channels welded to scroll position (parallax, the clip-path ellipse uncovers, camera moves), plus `IntersectionObserver` ×4. These re-play on every pass, forward and back, because they are scrubbed to the scrollbar rather than triggered. Siena's `[data-videoplayer='scrub']` is the hand-driven version of the same principle.
...
### Smooth scroll — near-universal, library tracks the stack

All three deep-read sites smooth the wheel; smoothing is effectively universal at this tier.
...
Lusion's renderer: WebGL is confirmed directly (`WebGLRenderingContext` ×3, `createProgram`) and the API surface is OGL-shaped (`Mesh` ×49, `Texture` ×39, `Program` ×2, `RenderTarget` ×2), so the OGL identity rests on that surface plus the studio's known stack — not on any literal `ogl` token, of which the bundle has none.
...
- The element-level anti-signals above hold mid-page too, confirmed by grep on both live bundles: no pale-tint fill, no frosted-nav-on-scroll, no nav border-bottom, no dead-amplitude hover (`scale(1.1)` mid-page too, via Siena `.previousnext-item:hover .full-img-w`), no one-hover-everywhere — button ≠ row ≠ image ≠ link ≠ nav, all routed through `cubic-bezier(.19,1,.22,1)`, `cubic-bezier(.23,1,.32,1)`, `cubic-bezier(.65,.05,0,1)`.

---

## Refuted

- **"Messenger — Awwwards Developer Site of the Year 2025 at `messenger.network`"** — false: the award is **SOTD Nov 10 2025 + Developer Award 8.21** at `messenger.abeto.co`. `messenger.network` 301-redirects to `genzee.io`.
- **"Lando's first fold is a 3D helmet on cream"** — false: the fold is a full-bleed frontal portrait photograph, with the helmet as a ghosted contour-line silhouette behind the face, plus a later "Helmets" gallery and the footer portrait. `<body>` computes `background: #282C20` with `#F4F4ED` text — the page is predominantly dark and the cream register is hero-and-footer only.
- **"Lusion v3 — 8.25 design"** — false label: 8.25 is the **overall** score; the design sub-score is **8.26**.
- **"Lusion reel names Oryzo / Space-NFTMarketplace / DD2024 / ChoChoWorld"** — false: decoding the quadrupled DOM `textContent` gives **OryzoAI**, **Spaace-NFTMarketplace**, **DDD2024** (12 D's ÷ 4) and **ChooChooWorld** (8 o's ÷ 4). The other six reel names hold.
- **"Lusion footer address: Suite 29 Marsh Street"** — false, a line-fusion misread: `footer-address-line` #1 carries "Suite" + "2" and the next line carries "9 Marsh Street". Shipped: **"Suite 2, 9 Marsh Street, Bristol, BS1 4AA, United Kingdom"**.
- **"Lando's preloader unmounts — no `[class*=load/preload/intro]` node survives"** — false as a blanket negative: `[class*="intro"]` still matches surviving content nodes `text-cta-short-intro` and `callout-socials-intro-w`. The curtain itself does unmount; the negative narrows to load / preload / curtain.
- **"Lusion / Messenger / Active Theory render on dark canvases"** — false for **Lusion**, whose body computes `rgb(255,255,255)` and whose v3 hero is a light-key studio environment. The dark-canvas grade holds for Active Theory (`#000`) and Messenger only.
- **"Lando's ellipse family includes `110% 110% at 50% 0%` and an `… at 50% 100%` end-state"** — false: both grep to **0 hits**. The verified family is `100% 0% at 50% 0`, `100% 120% at 50% 0`, `120% 100% at 50% 20%`, `120% 120% at 50% 0`, `70% 100% at 50% 0` — every member anchored at `50% 0` or `50% 20%`. The mechanic holds; the enumerated end-states were partly wrong.
- **"Lando ships 44 `data-rive` canvases and instantiates `new Rive` ×8+"** — false: the live build carries **17** `data-rive-artboard` mount points (3 literal `<canvas data-rive>`, 5 unique `.riv` files), and the only literal `new Rive` sits inside the Rive library's own deprecation-warning string — the site instantiates through a minified alias. `riveAllLoaded` ×11 confirms the runtime; the counts drift with an actively patched build and should be cited as ≥N ranges.
- **"Lando runs 31 concurrent `scrub:` channels"** — false: **29** (`scrub:!0` ×26, plus `scrub:0`, `scrub:0.5`, `scrub:1`).
- **"`toggleActions:"play"`, never `"play reverse"`"** — false as an absolute: Lando ships 2 × `toggleActions:"play none none reverse"`, both on WebGL decor (`uCursorIntensity`, wireframe `uOpacity` via `[data-gl-track="head"]`). The rule holds for content reveals, and the exception reinforces "content persists, decor reverses".
- **"Lando's entire 375KB CSS declares only 4 `transition-property` rules"** — false in detail: the two near-duplicate variants total **~366KB**, and each declares one `transition-property:color` plus one `transition-property:height` (2 unique rules, duplicated). Three `transition:` shorthands also exist (`transition:unset`, `background-color/color .1s`, `all .3s`), so "almost no CSS transitions" is directionally right but not literally 4 total.
- **"Lusion runs WebGL via OGL — 2 refs in the HTML"** — false as cited: both hits are the substring `ogl` inside *Google* and *googletagmanager*, and genuine `ogl`/`oframe` tokens are zero (the `oFrame` ×7 are substrings of `requestVideoFrameCallback`). WebGL stands (`WebGLRenderingContext` ×3, `createProgram`); the OGL identity rests on the API surface (`Mesh` ×49, `Texture` ×39, `Program` ×2, `RenderTarget` ×2) and the studio's known stack, not on the cited proof.
- **"Lando's pinned horizontal-track interlude (winner-verified)"** — unsupported at that tag, downgraded to shipped / observed: `horizontal` ×21, `xPercent` ×12 and every `pin`/`pinSpacing`/`pinType` token resolve to GSAP ScrollTrigger internals, not an isolable site-level horizontal-pin section. Confirming it requires driving the live site.
- **"Siena's Lenis is configured `syncTouchLerp:.075`, `touchInertiaMultiplier:35`"** — unsupported: those values are the Lenis constructor's destructuring defaults in the bundled signature, not proof of Siena's passed config. Lenis identity stands via the constructor and the `html.lenis` class family.
- **"Siena — Awwwards Site of the Month April 2025"** — false: the authoritative `sites_of_the_month/` listing shows Siena as the **March 2025** winner. The April figure comes from a secondary search summary.
- **Three committable Lando copy strings are not independently verified** — flagged, not cut: the hero status card ("NEXT RACE / SPA GP / MCLAREN F1 SINCE 2019"), the footer sponsor names (Quadrant, Bell Helmets, Tumi, Pure Electric, Android, PAP, Monster Energy, L7), and the footer nav-column labels (PAGES, FOLLOW ON).

**Evidence base.** Awwwards jury pages read directly, CSS and JS bundles downloaded and grepped verbatim, DOM read behind a `location.hostname` guard — 9–13 Jul 2026.
